...
export pkg_version="$VERSION.$PATCHLEVEL.$SUBLEVEL"
    last_version=$(curl -X POST https://api.mayuri.my.id/linux-xanmod -d '{"api_key": "'$api_key'", "type": "wsl2"}')
...
if [ "$pkg_version" != "$last_version" ]; then
    ...
    curl -X POST https://api.mayuri.my.id/linux-xanmod -d '{"api_key": "'$api_key'", "type": "wsl2", "pkg_version": "'$pkg_version'"}'
    export TELEBOT
